# How to use:
- Align robot with the VEX field grid and start the program
...
## How it works
- The robot first needs to figure out where the light sensor is located
- It does a 90 degree sweep of the side of the field that it is on
...
B --> C
B --> BY[yes]
BY --> BC[Store width and continue]
BC --> C
C{rotaed 90 degrees} --> CN[no]
CN --> B
```
